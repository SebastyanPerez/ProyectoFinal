document.addEventListener("DOMContentLoaded", function () {
    ...
    function filterProducts() {
      ...
      const selectedBrands = getSelectedValues("brand"); // Agregar filtro de marcas
  
      productCards.forEach((card) => {
        const productPrice = parseInt(card.getAttribute("data-price"));
        const productColor = card.getAttribute("data-color");
        const productBrand = card.getAttribute("data-brand").toLowerCase(); // Agregar marca
        ...
        const matchesBrand =
          selectedBrands.length === 0 || selectedBrands.includes(productBrand); // Filtrar por marca
